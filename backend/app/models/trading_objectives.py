...
def clear_resolved_proposals() -> int:
    """Remove all rejected and executed proposals. Returns count removed."""
    proposals = load_proposals()
    keep = [p for p in proposals if p.get("status") in ("pending", "approved")]
    removed = len(proposals) - len(keep)
    _save_json(PROPOSALS_PATH, keep)
    return removed
